...
2. 构建cube：
   ...
   web ui:
   kylin_streaming_cube  点击 action -> build 进行构建cube -> 进入Monitor界面查看状态
   rest Api:
   curl -X PUT -u ADMIN:KYLIN -H "Content-Type: application/json;charset=utf-8" -d '{ "sourceOffsetStart": 0, "sourceOffsetEnd": 9223372036854775807, "buildType": "BUILD"}' http://localhost:7070/kylin/api/cubes/kylin_streaming_cube/build2
...
3. 执行sql查询： build成功后，-> Insight，输入sql语句
   ...
   rest Api:
   curl -X POST -u ADMIN:KYLIN -H "Content-Type: application/json" -d '{ "sql":"select * from KYLIN_STREAMING_TABLE limit 2", "project":"learn_kylin" }' http://localhost:7070/kylin/api/query
